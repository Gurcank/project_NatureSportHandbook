...
export default function AnimalSubCategoryPage() {
  ...
  if (key === 'mammals') {
    ...
    const leafPages = [
      {
          ...
          <div className="pr-4 md:pr-8">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {mammalAnimals.map((animal, index) => (
                <article
                  key={animal.id}
                  className="rounded-2xl border border-[#8d6c40]/25 bg-[#f5e6bd]/70 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.35)]"
                >
                  <p className="mb-1 text-xs uppercase tracking-[0.22em] text-[#5a5a5a]/60">
                    0{index + 1}
                  </p>
                  <h3
                    className="text-xl font-semibold text-[#5a5a5a]"
                    style={{
                      fontFamily: 'var(--font-kalam), "Segoe Print", "Bradley Hand", cursive',
                    }}
                  >
        ...
        right: (
          <div className="relative h-full min-h-[29rem] sm:min-h-[31rem]">
            ...
            <p
              className="max-w-xl text-base leading-7 text-[#5a5a5a]"
              style={{ fontFamily: 'var(--font-kalam), "Segoe Print", "Bradley Hand", cursive' }}
            >
              {language === 'tr'
              ...
              left: (
                <div className="pr-4 md:pr-8">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <article className="rounded-2xl border border-[#8d6c40]/25 bg-[#f5e6bd]/70 p-4">
                      <p className="text-xs uppercase tracking-[0.22em] text-[#5a5a5a]/60">
                        Habitat
                      </p>
              ...
              right: (
                <div className="relative h-full min-h-[29rem] sm:min-h-[31rem]">
                  <div className="rounded-3xl border border-[#8d6c40]/20 bg-[#f5e6bd]/65 p-5">
                    ...
                    <h3 className="mt-2 text-2xl font-semibold text-[#5a5a5a]">
                      {firstMammal.name}
                    </h3>
                    <p className="mt-2 text-sm leading-6 text-[#5a5a5a]/85">
                      {language === 'tr'
                        ? 'Bu yapraktan sonra diğer memeli yaprağına geçebilirsin.'
                        : 'You can move to the next mammal leaf after this one.'}
                    </p>
                  </div>
              ...
              right: (
                <div className="relative h-full min-h-[29rem] sm:min-h-[31rem]">
                  <div className="rounded-3xl border border-[#8d6c40]/20 bg-[#f5e6bd]/65 p-5">
                    ...
                    <p className="mt-2 text-sm leading-6 text-[#5a5a5a]/85">
                      {language === 'tr'
                        ? 'Sonraki tıklama ilk yaprağa döner.'
                        : 'The next click returns to the first leaf.'}
                    </p>
                  </div>
    ...
    return (
      <div
        className={`relative flex-1 mx-auto w-full max-w-[88rem] px-4 py-4 sm:py-5 transition-all duration-500 ${isTurningPage ? 'opacity-0 translate-x-10 scale-[0.985]' : 'opacity-100 translate-x-0 scale-100'}`}
      >
        <NotebookSpread
          leftNotes={[]}
          rightNotes={[]}
    ...
    <div className="flex-1 mx-auto w-full max-w-6xl px-4 py-12">
      <section className="rounded-2xl border border-teal-300/40 bg-black/35 p-6 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {text.items.map((item) => {
            ...
            return (
              <article
                key={item}
                className="rounded-xl border border-teal-200/35 bg-teal-900/30 p-4"
              >
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href="/nature/animals"
            className="rounded-lg border border-teal-200/60 bg-teal-100/15 px-4 py-2 font-medium text-teal-100 transition hover:bg-teal-100/25"
          >
            {language === 'tr' ? 'Hayvan kategorilerine don' : 'Back to animal categories'}
          </Link>
          <Link
