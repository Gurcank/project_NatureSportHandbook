...
function MammalCard({ animal, language }: { animal: MammalEntry; language: Language }) {
  return (
    <article className="flex gap-3.5 rounded-[1.4rem] px-1 py-1">
      ...
      >
        <p className="text-[0.96rem] font-semibold leading-6.5 text-inherit">
          {animal.scientificName}
        </p>
        <p className="mt-0.5 text-[0.9rem] leading-6.5 text-inherit">• Diet : {animal.diet}</p>
        <p className="mt-0.5 text-[0.9rem] leading-6.5 text-inherit">
...
export default function MammalsHandbook() {
  ...
  return (
    <div
      className={`relative flex-1 mx-auto w-full max-w-[88rem] px-4 py-4 sm:py-5 transition-all duration-500 ${turning ? (direction === 'next' ? '-translate-x-8 opacity-0 scale-[0.985]' : 'translate-x-8 opacity-0 scale-[0.985]') : 'translate-x-0 opacity-100 scale-100'}`}
    >
      <NotebookSpread
        leftNotes={[]}
        rightNotes={[]}
